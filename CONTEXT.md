...
**版本闸（version gate）**：
改一个**已存在**的文件，要求这次对话里有它**当前版本**的一次 `Read`——`src/agents/readBeforeWrite.ts`。
**只管 `Edit`**：`Write` 本来就永不覆盖（`src/tools/mutating.ts:48-54`），
**一条已经被更严的规则覆盖的路不该再被一条更松的规则覆盖**，第二条只会让第一条更难读。
判据是二值的：**文件存在 且 没有当前版本的读标记**。哈希不出来时**放行**——
**闸的失效不该比没有闸更糟**。见 `docs/adr/0008`。
...
**读标记（read mark）**：
一次 `Read` 在它的 `ToolMessage` 上留下的「哪个文件 + 当时的 sha256」。
**盖在 `additional_kwargs` 上，与钉住标记同一个位置**——所以**摘要吃掉那条读结果，标记跟着没，
闸就再也过不去**。🔑 **这不是巧合**：投影决定模型看得见什么，闸的状态就该活在模型看得见的那份里。
**写永不刷新它**：任何一次成功的写都改变哈希，因而作废此前所有的读。
