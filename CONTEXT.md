...
**闸的失效不该比没有闸更糟**。见 `docs/adr/0008`：装不装这类反馈机制按**判据 5+1** 判——
前五项判**设计**（确定性观测 · 精确触发 · 结构化后果 · 一条诚实的出路 · 存续期匹配作用期），
第六项判**部署**（**上岗验收**：亲眼看它红过一次、绿过一次才算装上，2026-08-31 增补）。
本闸自己的红绿两枪是 `repro/41`；三条增补（Write 盖章、Edit 盖章、第六项）都在那份 ADR 里。
