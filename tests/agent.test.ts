...
import { createAgentGraph, RECURSION_LIMIT } from "@/agent";
...
// The lap: model asks for a tool, the tool runs, its result goes back into state,
// and the model is called again. `.addEdge("tools", "llmCall")` is the only thing
// making that second call happen.
...
test("advertises the tools to the model on every call", async () => {
  ...
  for (const request of requests) {
    const tools = (request as unknown as { tools?: { function: { name: string } }[] })
      .tools;
    expect(tools?.map((tool) => tool.function.name)).toEqual(["Read", "Glob", "Grep"]);
  }
});
