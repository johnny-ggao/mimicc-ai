...
const RETIRED: Record<string, string> = {
  // 今天是空的，**而这不是遗憾，是记录**：第一个候选人 `45-how-many-laps-fit-in-the-limit.ts`
  // 被上面那两条判据挡回去了。它看起来完全符合退役——问题过期、常数被删、跑十分钟什么都不
  // 回答——但顺着它的工具旁挂查下去，**它挂住是因为回合预算和 loop guard 在装配好的图里
  // 根本不响**（`afterModel` 从第二圈起拿到的最后一条消息是 `ToolMessage`，判据那两行一次
  // 都没执行到）。**它不是没用了，它在报警。**
  //
  // 🔑 所以判据的第二条要按字面读：**「它量的那样东西没了」不等于「它现在跑不动了」。**
  // 45 号量的常数确实没了，但它跑不动的原因是另一回事——而那个原因是一个活的缺陷。
};
