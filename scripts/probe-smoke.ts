...
/**
 * 花钱那几个的上限，短得多，而且**杀掉它们是正常收尾不是失败**。
 *
 * 它们打的是死端口，而死端口是一种失败——`AsyncCaller` 会把它重试 6 次并退避
 * （`repro/README.md` 顶上那条警告，实测一次 400 打了服务器 7 遍）。实测 `repro/19`
 * 这样能跑 5 分钟以上。而判据在第一次连接被拒时就已经满足了：**它活到了发请求那一步。**
 * 后面那些重试不再回答任何问题，纯等。
 */
