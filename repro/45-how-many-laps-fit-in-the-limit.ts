 ...
 * ## 为什么退役
 *
 * ADR 0009（commit `5fea7ee`，2026-08-27）把**步数预算整个删掉了**：`RECURSION_LIMIT`
 * 今天是 `1_000_000` 的格式占位（langgraph 硬性要求一个 ≥1 的有限数），**撞不到**。
 * 这个探针的观测面②是「让模型永远只调工具，数**撞顶前**跑了几圈」——**那个顶没了。**
 *
 * 实测（2026-08-28，退役当天）：单独跑 **10 分 17 秒**，父进程最后印的是
 * *「子进程没交回东西」*，**退出码 0**。所以它今天既不是「红」也不是「答对」，
 * 而是 `README.md` 顶上警告的第三种：**跑得起来，但什么都不回答**——
 * 把冒烟的上限抬高也救不了它。
 *
 * ⚠️ **这不是腐烂，所以不修**（分辨法见 `README.md`「退役」一节：把接口改回去它就能答吗？
 * 不能——被它量的那个常数本身没了）。它的结论已经落在 ADR 0009 与 CONTEXT.md「回合预算」，
 * 而**下面这段记录里最值钱的一条**——「守卫判的是病态不是长度，一个老实跑 12 个不同往返的
 * 任务一个守卫都不会响」——正是 ADR 0009 删掉步数预算的理由之一。
 *
 * 登记在 `scripts/probe-smoke.ts` 的 `RETIRED`：**只列不跑**，名字每次冒烟照样占一行。
 *
 * ## 为什么问这一问
 *
