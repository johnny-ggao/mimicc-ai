 ...
 *
 * ## 🔴 它今天跑不完，而那不是它的错（2026-08-28）
 *
 * 题面是过期的：ADR 0009（`5fea7ee`）把步数预算整个删了，`RECURSION_LIMIT` 今天是
 * `1_000_000` 的格式占位，**撞不到**。所以「撞顶前跑了几圈」这个问题已经没有答案了，
 * 我一度据此把它退役。**那是读反了。**
 *
 * 顺着它的工具旁挂（`.mimicc/probe-45/.state/probe-45.tools.jsonl`）查下去：
 * 单独跑 10 分 17 秒、**5583 圈**、`intent` 与 `settlement` 各 5583 条全部善终，
 * **而旁挂没有被清空**——按 `repro/21` 量过的性质，跑完的回合会清空它。
 * 也就是说那个回合**从来没有正常收尾**，而不是「跑完了只是没答案」。
 *
 * 🔴 缩小复现（墙钟压到 3 秒、探针自己给 20 秒闸）：**1417 圈全是同一个 `Read`，
 * `onCap` 0 次**——回合预算和 loop guard 一个都没响。给真代码加打印看到的是：
 * `beforeAgent` 只跑一次（对）、`afterModel` 每圈都跑（对）、`elapsed` 涨到预算的 6.6 倍，
 * **但从第二圈起它拿到的最后一条消息是 `ToolMessage` 而不是 `AIMessage`**，于是
 * `turnBudget.ts` 那句 `if (last === undefined || !AIMessage.isInstance(last)) return;`
 * 每圈提前返回，判据那两行一次都没被执行到。
 *
 * 🔑 **所以这个探针今天挂住，是它在报警，不是它烂了。** 它是这个仓库里唯一一个把真图推进
 * 一个**老实的无限工具循环**、然后看谁会拦住它的东西——ADR 0009 说「真正的护栏 = 回合预算
 * + loop guard + stall guard + 墙钟」，而它量出来的是：**没有人拦。**
 * 后续在 `repro/51`。
 *
 * ## 为什么问这一问
 *
