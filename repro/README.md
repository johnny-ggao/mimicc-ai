...
| 🔴 `45-how-many-laps-fit-in-the-limit.ts`（**今天跑不完，见下**）| `RECURSION_LIMIT = 48` 等于几圈。🔑 **8 圈，每圈 6.0 个节点**（`1 agent + 4 afterModel + 1 tools`），而那 4 个 afterModel **全是守卫本身**（LoopGuard／EmptyReplyGuard／Clarify 门／确认门）。🔴 **`loop.ts:41-48` 那段「守卫会先于上限触发」说反了一半**：守卫判的是病态（同调用重复 3/5、连续失败 3），不是长度——**一个老实跑 12 个不同往返的任务一个守卫都不响**。起因是外部 benchmark 实测撞顶（只走了 7 圈）。🔴 **2026-08-28：题面过期了（ADR 0009 删掉了那个常数），但它今天跑不完是另一回事——回合预算与 loop guard 在装配好的图里根本不响**（`afterModel` 从第二圈起拿到的是 `ToolMessage`，判据提前返回）。**它在报警，不是烂了**；后续见 `repro/51` | 否（本地 stub） |
...
`45-how-many-laps-fit-in-the-limit.ts` 被下面这两条判据**挡回去了**：它看起来完全符合退役
——问题过期（ADR 0009 删掉了 `RECURSION_LIMIT` 那个常数）、跑十分钟什么都不回答——
但顺着它的工具旁挂查下去，**它挂住是因为回合预算和 loop guard 在装配好的图里根本不响**。
**它不是没用了，它在报警。** 判据的第二条要按字面读：
**「它量的那样东西没了」不等于「它现在跑不动了」**——两件事在 45 号身上恰好同时为真，
而只有前一件是退役的理由。
