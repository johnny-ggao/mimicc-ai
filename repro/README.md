...
| ⏸️ `45-how-many-laps-fit-in-the-limit.ts`（**已退役**，2026-08-28）| `RECURSION_LIMIT = 48` 等于几圈。🔑 **8 圈，每圈 6.0 个节点**（`1 agent + 4 afterModel + 1 tools`），而那 4 个 afterModel **全是守卫本身**（LoopGuard／EmptyReplyGuard／Clarify 门／确认门）。🔴 **`loop.ts:41-48` 那段「守卫会先于上限触发」说反了一半**：守卫判的是病态（同调用重复 3/5、连续失败 3），不是长度——**一个老实跑 12 个不同往返的任务一个守卫都不响**。起因是外部 benchmark 实测撞顶（只走了 7 圈）。🔴 **2026-08-28 订正两次**：题面过期（ADR 0009 删了那个常数）；而它一度跑十分钟不回答，**是它自己的 stub 恒用同一个 `id` 造成的**（见下「stub 的 id 必须唯一」），**不是被测程序的毛病**——`repro/51` 四格证过。id 修好后 **0.15 秒、5 圈、`loopGuard` 停住** | 否（本地 stub） |
...
一个探针可以走到尽头——**它的问题答完了，而被它量的那样东西已经不在了**。

名单上第一个、也是今天唯一一个是 `45-how-many-laps-fit-in-the-limit.ts`。
**它在判据前面来回了三趟，前两趟都栽在同一件事上：没弄清它为什么跑不动就下判断。**

1. 它看起来完全符合退役：问题过期（ADR 0009 删了 `RECURSION_LIMIT` 那个常数）、
   跑十分钟什么都不回答。**判退役——错了**，因为第二条理由没查清。
2. 查下去发现它的旁挂没被清空，于是判「它在报一个活的缺陷」，还写了一整份根因分析。
   **也错了**——真因是**它自己的 stub 恒用同一个 `id`**（见下一节），被测的程序好好的。
3. 修好 id：**0.15 秒跑完**。到这一步才看清真正的理由：**两个观测面都死了**
   （`getGraph()` 拿不到、上限撞不到），**而它照样印一个错的结论**
   （「真实任务能用的往返次数 = 5」——5 是 `loopGuard` 对重复调用的阈值）。
   **退役，理由里一个字都不提「它跑不动」。**

🔑 **所以判据前面还要加一句，它比判据本身更容易被跳过**：
**先修腐烂，再判退役。** 一个跑不动的探针，它跑不动的原因必须先弄清楚，
否则「退役」就是把一个没被理解的现象扫到地毯下面。
⚠️ 反过来也成立：**修好之后它可能仍然该退役**——45 号就是。修腐烂不是为了留住它，
是为了让退役的理由是真的。

退役**不是删文件**（证据不能只存在于一台机器上，见本文顶上），也**不是从冒烟里拿掉**
（那等于把「没人知道它死了多久」再来一次）。退役的定义是**只列不跑**：文件留在 git 里，
名字每次冒烟照样占一行，旁边写着为什么。🔑 **看得见才叫退役，看不见就叫划掉。**
...
⚠️ **「它现在跑不起来」不是退役的理由，那是腐烂**——腐烂要修，那是冒烟存在的全部意义。
分辨的方法是问一句：**把接口改回去它就能答了吗？** 能，就是腐烂；不能，才是退役。

⚠️ **退役的探针必须同时登记两处**：这张表里那一行标上「已退役」，
**以及 `scripts/probe-smoke.ts` 的 `RETIRED`**（理由写在那里，冒烟每次都会印出来）。
只改表不改 `RETIRED` 的后果是它照跑照红；只改 `RETIRED` 不改表的后果是这张表在说谎。
冒烟自己会挡住第三种错法：**`RETIRED` 里指向一个不存在的文件时它直接报红**——
一份指着空气的退役名单本身就是腐烂，而且是安静的那种。
...
⚠️ **stub 每次回复的 `id` 必须唯一**（`chatcmpl-1`、`stub-${n}`……）。
`add_messages` 是**按 id upsert** 的：所有回复都叫 `stub` 的话，每条新的 AI 消息会
**覆写**上一条，整段历史长成 `human > ai > tool > tool > tool …`——**永远只有一条 `ai`**。
于是所有判「最后一条消息」的机制（回合预算、`loopGuard`）**全部假性哑掉**，
探针会跑到天荒地老，而且**看起来完全像被测程序的缺陷**。
2026-08-28 在 `repro/45` 上实际发生：十分钟不回答，我据此写了一份「护栏在真图里不响」的
报告，**整份都是错的**（`repro/51` 四格推翻）。
🔑 **同一条教训的两个方向**：上一次是 stub 太像成功（`repro/36` 照着 `finish_reason:
"length"` 造，把一个死了四天的机制照成绿的），这次是 stub 造出一个假的失败。
**解药都是给判据配一个对照格。**
