...
🔴 **今天这份名单是空的，而这不是遗憾，是记录。** 第一个候选人
`45-how-many-laps-fit-in-the-limit.ts` 在判据前面来回了两趟，**两趟都栽在同一件事上：
没弄清它为什么跑不动就下判断。**

1. 它看起来完全符合退役：问题过期（ADR 0009 删了 `RECURSION_LIMIT` 那个常数）、
   跑十分钟什么都不回答。**判退役——错了**，因为第二条理由没查清。
2. 查下去发现它的旁挂没被清空，于是判「它在报一个活的缺陷」。**也错了**——
   真因是**它自己的 stub 恒用同一个 `id`**（见下一节），被测的程序好好的。
3. 修好 id：**0.15 秒跑完**。到这一步，退役才是**干净的**——理由只剩「题面过期」这一条。
