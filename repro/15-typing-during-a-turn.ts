...
async function main(): Promise<void> {
  ...
  check(
    "回归护栏：门只消费门出现之后敲的行（2026-08-19 修）",
    !tty.eaten,
    `TTY ${tty.eaten ? "🔴 又被吃了" : "没被吃"} / 管道 ${pipe.eaten ? "被吃（故意）" : "没被吃"} —— ` +
      (tty.eaten
        ? "**这个 bug 回来了。** 用户敲给模型的话又落地成了对 Bash 的否决。"
        : "那句话不再变成一条拒绝，它排在门后面成为下一个回合。\n   " +
          "⚠️ **管道一侧是故意松的**：管道没有「敲下去的那一刻」——整个脚本在进程启动前就写好了，" +
          "顺序是作者定的，也没有一个会随手敲回车的人要保护。在那里强制到达顺序，" +
          "只会让「从脚本里答一道门」变得不可能，而这个程序的探针与测试正是这么驱动的。"),
  );
